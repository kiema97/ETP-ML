...
mdl_filname <- dir("outputs/BaggingTrees",pattern = ".rds")
...
for(station in stations){
  j <- j+1
  mdls[[station]] <- readRDS(file.path("outputs/BaggingTrees",mdl_filname[j]))
...
write.table(x =mdl_metrics ,file ="outputs/BaggingTrees/bagging_metrics.csv" ,
