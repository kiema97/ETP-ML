...
mdl_filname <- dir("outputs/SVM_RBF",pattern = ".rds")
...
for(station in stations){
  j <- j+1
  mdls[[station]] <- readRDS(file.path("outputs/SVM_RBF",mdl_filname[j]))
...
write.table(x =mdl_metrics ,file ="outputs/SVM_RBF/svm_rbf_metrics.csv" ,
