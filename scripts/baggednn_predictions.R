...
mdl_filname <- dir("outputs/BaggedNN",pattern = ".rds")
...
for(station in stations){
  j <- j+1
  mdls[[station]] <- readRDS(file.path("outputs/BaggedNN",mdl_filname[j]))
...
write.table(x =mdl_predictions ,file ="outputs/BaggedNN/baggednn_predictions.csv" ,
