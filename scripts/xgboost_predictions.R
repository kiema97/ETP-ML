...
mdl_filname <- dir("outputs/XGBosst",pattern = ".rds")
...
for(station in stations){
  j <- j+1
  mdls[[station]] <- readRDS(file.path("outputs/XGBosst",mdl_filname[j]))
...
write.table(x =mdl_predictions ,file ="outputs/XGBosst/xgboost_predictions.csv" ,
