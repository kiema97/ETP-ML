...
#mdl_filname <- dir("outputs/SVM_POLY",pattern = "^svm_poly_model_v2_.*\\.rds$")
mdl_filname <- dir("outputs/SVM_POLY",pattern = ".rds")
mdls <- list()
j <- 0
stations <- unique(train_data$stations)
#stations <- c("bobo","fada","gaoua","ouaga")
for(station in stations){
  j <- j+1
  mdls[[station]] <- readRDS(file.path("outputs/SVM_POLY",mdl_filname[j]))
...
write.table(x =mdl_metrics ,file ="outputs/SVM_POLY/svm_poly_metrics.csv" ,
