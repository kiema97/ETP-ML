...
mdl_filname <- dir("outputs/GLMNET",pattern = ".rds")
...
for(station in stations){
  j <- j+1
  mdls[[station]] <- readRDS(file.path("outputs/GLMNET",mdl_filname[j]))
...
write.table(x =mdl_predictions ,file ="outputs/GLMNET/glmnet_predictions.csv" ,
