...
mdl_filname <- dir("outputs/MARS",pattern = ".rds")
...
for(station in stations){
  j <- j+1
  mdls[[station]] <- readRDS(file.path("outputs/MARS",mdl_filname[j]))
...
write.table(x =mdl_metrics ,file ="outputs/MARS/mars_metrics.csv" ,
