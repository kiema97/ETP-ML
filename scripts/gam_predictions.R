...
mdl_filname <- dir("outputs/GAM",pattern = ".rds")
...
for(station in stations){
  j <- j+1
  mdls[[station]] <- readRDS(file.path("outputs/GAM",mdl_filname[j]))
...
write.table(x =mdl_predictions ,file ="outputs/GAM/gam_predictions.csv" ,
